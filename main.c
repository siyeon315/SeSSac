...
volatile float distance_array[NUM_MEASUREMENTS] = {0};
...
volatile Mode current_mode = MEASURE_10;
...
void update_distance_array(float new_distance)
{
    distance_array[measurement_cnt] = new_distance;
    measurement_cnt++;

    if (measurement_cnt >= NUM_MEASUREMENTS) 
    {
        float sum = 0;
        ...
        float avg_distance = sum / NUM_MEASUREMENTS;
        Printf("Average Distance: %0.2f cm measurement_cnt : %d\r\n", avg_distance, measurement_cnt);
...
int main(void)
{
  ...
  while (1)
  {
		switch (current_mode)
        {
            ...
            case WAIT_BUTTON:
								Printf("current mode is WAIT_BUTTON\r\n");
								//current_mode = MEASURE_CONTINUOUS;
                if (Door_Open)
                {
                    Door_Open = 0;
                    Printf("Button Pressed! Restarting Measurement...\r\n");
										current_mode = MEASURE_CONTINUOUS;
                }
                break;
